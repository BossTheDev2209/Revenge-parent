...
**Edit more — ✅ lock + ทำแล้ว 27 ก.ค.** (กติกาจาก user: "edit more ก็กิน mental ตามกฎ mental system")

| เรื่อง | กติกา |
|--------|-------|
| ทำอะไร | ตัดคลิป**เดิม**ซ้ำอีกรอบ — **ไม่สร้างคลิปใหม่** (`FinishEdit{score, redo=true}`) |
| คะแนน | เก็บ **max(เดิม, ใหม่)** ตัดแย่ลงไม่โดนลด (ไม่งั้นเสียใจฟรีสองต่อ) แล้วคำนวณ tier ใหม่จากคะแนนนั้น |
| ราคา | **`clipEdit` −20 ใจ ทุกรอบ** + นับ `EditStreak5` ทุกรอบ (5 ติด = drain ×1.5) — ไม่มี cap จำนวนรอบ ใจคือ limit เอง |
| ไม่มีคลิปค้าง | `redo` ลอย (คลิปอัปไปหมดแล้ว) = ตัดใหม่ปกติ ไม่พัง |
| เก็บสถิติ | `clip.edits` = ตัดไปกี่รอบ (ไว้ใช้กับกราฟ Feedback ทีหลัง) |
