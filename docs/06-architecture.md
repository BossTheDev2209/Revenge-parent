...
### ⚠️ `StateChanged` เป็น event ไม่ใช่ property — push ที่มาก่อน `:Connect()` **หายถาวร**

server ไม่ push ซ้ำถ้า state ไม่เปลี่ยน → ใครก็ตามที่ connect ช้ากว่า push แรกๆ **ไม่มีวันได้ข้อมูลนั้นอีกเลย**
ไม่มี error ไม่มี warning — แค่ค่าไม่มา แล้ว UI นั้นค้างอยู่ที่ค่าตั้งต้นเงียบๆ

> **เคสจริง 10 ส.ค. 2569 — เมนู "เหมือนไม่เคยเล่นมาก่อน" (เซฟหาย กดสร้างใหม่ไม่ได้):**
> `MenuUI.open()` อยู่บรรทัด ~199 ของ `Main.client.luau` แต่ `StateChanged:Connect` อยู่บรรทัด ~340
> (ห่างกัน 140 บรรทัด — ต้อง require 20+ โมดูล + `WaitForChild("UI")` ก่อน)
> ฝั่ง server ตอน `PlayerAdded` push 2 รอบ: ทันที (ยังไม่มี `slotList`) แล้ว push อีกรอบหลัง prefetch
> `listSlots` เสร็จ — **ซึ่งใช้แค่ ~0.3 วิ** ถ้ารอบสองมาถึงก่อน client รันถึงบรรทัด connect = **หายเงียบ**
>
> ผลลัพธ์ต่อเนื่องเป็นทอด: `slotList` ไม่ถึงเมนู → ช่องเซฟว่างหมด → `slotsReady` ไม่เคยเป็น `true`
> → กดช่องเซฟแล้ว `return` ทิ้ง (guard กันเดา NewGame ทับเซฟจริง) → **ออกได้ทางเดียวคือปุ่ม "เล่นต่อ"**
> ที่มี path `pendingContinue` ยิง `RequestSlots` ใหม่
>
> **ยิ่ง DataStore เร็ว ยิ่งพังบ่อย** (prefetch ชนะ client boot) ตอน DataStore ช้า ~1.6 วิ กลับทำงานปกติ
> → อาการเลยเป็นๆ หายๆ และ**หลอกให้ไปโทษ DataStore** ทั้งที่ตอนเกิดอาการ `listSlots` คืนเซฟครบ 4 ช่อง
> ใน 0.30 วิ `slotsFailed = false` และ `slotList` ส่งถึง client ครบถ้วน
>
> **แก้:** ดัก `StateChanged` ไว้ตั้งแต่ก่อน `MenuUI.open()` เก็บ `latestState` + ส่งต่อ `MenuUI.updateSlots`
> แล้วปลดตัวดักตอนตัวรับหลักพร้อม (`earlyStateConn` ใน `Main.client.luau`)

**กติกาสำหรับ UI ทุกตัวที่กินข้อมูลจาก `StateChanged`:**

| กฎ | เหตุผล |
|---|---|
| **connect ก่อน แล้วค่อยเปิด UI** — อย่าเปิด UI ที่รอข้อมูลก่อน listener พร้อม | push ที่มาระหว่างนั้นหายถาวร |
| ถ้าเลี่ยงไม่ได้ ให้ UI **seed จาก state ล่าสุดที่เก็บไว้** ตอนเปิด (`deps.getState()`) | กันเคสที่ข้อมูลมาถึงก่อน UI เปิด |
| อย่าตีความ "ไม่มีข้อมูล" = "ไม่มีของ" | ยังไม่มา ≠ ว่าง · เดาผิดฝั่งนี้ = **เขียนทับเซฟจริง** |
| debug อาการ "ค่าไม่ขึ้น" ให้เช็ค**ลำดับ connect เทียบเวลา push** ก่อนสงสัย DataStore/network | เสียเวลาไล่ผิดทางมาแล้ว |
