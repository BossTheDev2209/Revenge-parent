...
**HUD ซ่อนตอนอยู่เมนู** (แก้ 29 ก.ค.) — `Gui_HUD.Enabled = false` เป็นค่าเริ่มต้น เมนูเปิดให้ตอนกดเล่นจริง ถ้าบอสสร้าง HUD ใหม่เอง **อย่าลืมตั้ง Enabled = false**
...
### PC UI (7 แอป) อยู่ไหน — ไม่มีใน StarterGui ตั้งใจ

จอคอม + แอป (Bank/Upload/Feedback/Calendar/Message/Manage/Shop/Edit) **ไม่ได้เป็น ScreenGui ลอยหน้าจอ** — มันเป็น **SurfaceGui แปะบนจอคอมจริงในโลก** ที่โค้ดสร้างตอน**เดินไปกด E ที่ `Interact_pcMonitor`** (PDF หน้า 5: "เข้าโดยไปนั่งที่ seat")

→ เห็นได้เฉพาะตอนเข้าไปใช้คอม ไม่ใช่ลอยตลอดเวลา · แต่ละแอปโผล่ทีละอันตอนกดไอคอน — **นี่คือพฤติกรรมที่ถูกแล้ว** (contextual UI ตามที่บอสอยากได้)

อยากแต่งหน้าตาแอปพวกนี้ใน Studio เหมือน HUD → ต้องแปลงให้เก็บ template ไว้ใน `ReplicatedStorage` แล้วโค้ด clone มาแปะจอ (งานแยกต่อแอป) บอกได้ว่าจะเริ่มแอปไหน
