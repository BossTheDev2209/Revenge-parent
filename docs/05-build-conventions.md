...
### ⚠️ บทเรียนจริง (10 ส.ค. 2569) — ARC รายกล่องไม่พอ ต้องมี Root ครอบทั้งชุด

`Gui_HUD` เคยทำตามตารางล่างครบทุกข้อ (Scale + ARC ทุกกล่อง) **แต่ยังเพี้ยนบนจอ aspect อื่น** เพราะแต่ละกล่องตั้ง Position เป็น Scale **สองแกนอิสระ** (X ผูกความกว้างจอ, Y ผูกความสูงจอ แยกกัน) — จอ aspect เปลี่ยน ระยะห่างระหว่างกล่องขยับคนละอัตรากับตัวกล่องเอง องค์ประกอบทั้งหน้าเลยบิด (ไม่ใช่แค่กล่องเดียวยืด)

**fix ที่ใช้จริง — ครอบด้วย Root frame เดียว:**
1. สร้าง `Frame` ชื่อ **`Root`** ตรงใต้ `ScreenGui` เลย — โปร่งใส, `AnchorPoint (0.5,0.5)`, `Position (0.5,0.5) scale`, `Size (1,1) scale`
2. ใส่ `UIAspectRatioConstraint` ที่ `Root` ตัวเดียว — `AspectRatio` = สัดส่วนจอที่จัดเลย์เอาต์ไว้ตอนออกแบบ (เช่น 16:9 ≈ 1.78, ไม่ใช่ 1:1)
3. **ย้ายชิ้นส่วน HUD ทั้งหมดเข้าไปเป็นลูกของ `Root`** (ไม่ใช่ลูกตรงของ ScreenGui อีกต่อไป)
4. ARC รายกล่อง (ตารางล่าง) ยังต้องมีต่อ — กันปุ่ม/ไอคอนแต่ละชิ้นเบี้ยว **แต่ ARC เดี่ยวๆ แก้ไม่ได้ที่ทั้งชุดขยับสัมพันธ์กันผิด** ต้องมี Root ด้วยเสมอ

Root ทำให้ทั้งแผงย่อ/ขยายพร้อมกันเป็นก้อนเดียว (letterbox ขอบว่างถ้าจอ aspect ต่างจากที่ออกแบบ) แทนที่จะยืดแต่ละชิ้นเพี้ยนคนละทาง — โค้ดอ่านตัวอย่าง `HUD.luau` (`HUD.bind` มองหา `gui.Root` ก่อน หา `Root` ไม่เจอค่อย fallback ไปหาที่ตัว `gui` ตรงๆ)

**⚠️ ของใน `.rbxl` ไม่ใช่ `src/` — สร้าง/ย้าย Root ใน Studio แล้วต้อง Ctrl+S + publish เอง ไม่งั้น revert (git ไม่ track)**

| กฎ | ทำไม |
|----|------|
| **มี `Root` (ARC ครอบทั้งชุด) ก่อนวางกล่องลูกทุกครั้ง** | กันทั้งแผงบิดสัมพันธ์กัน ไม่ใช่แค่กล่องเดียวยืด (ดูบทเรียนบน) |
| **Size ใช้ Scale ไม่ใช่ Offset** (`UDim2.fromScale`) | Offset = pixel คงที่ → จอเล็กล้น จอใหญ่จิ๋ว |
| **ใส่ `UIAspectRatioConstraint` ทุกกล่อง/ปุ่มด้วย** | กัน scale แล้วยืดผิดสัดส่วนของตัวมันเองบนจอ aspect ต่างกัน |
...
**เช็คทุกครั้ง:** Studio → **Test → Device** สลับ iPhone แนวตั้ง / แท็บเล็ต / จอ 16:9 ดูตาจริงก่อนปิดงาน — resize หน้าต่าง Studio โดยตรงก็เห็นผลไวกว่า (ไม่ต้องเปิด Device mode)
