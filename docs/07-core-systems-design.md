...
### 3.4 Edit QTE — มินิเกมตัดคลิป

**Spec จาก design:** ปุ่ม 20 อัน จากชุด Q W E A S D Z X C, อันละ 1.5 วิ (รวม 30 วิ), แม่นแค่ไหน = คะแนน, เต็ม 200 → tier
...
→ ทีละปุ่ม: โชว์ตัวอักษร + วงแหวนหด 1.5 วิ
   กดถูก: คะแนน 0-10 ตามจังหวะใกล้กลาง | กดผิด/ไม่กด: 0
→ ครบ 20 → รวมคะแนน → ยิง Action {type="FinishEdit", score=จำนวน}
→ server: clamp 0-200 → Formulas.vidqTier → เก็บ clip ลง state.clips → push()
```

- **ทั้งมินิเกมอยู่ client ตัวเดียว** (`UI.Apps.Edit`) — server รับแค่คะแนนจบ (singleplayer ไม่ต้องกัน cheat)
- ปุ่มไหนโผล่เมื่อไหร่ = สุ่มจาก list ใน `VidQConfig.qte = {keys={...}, count=20, window=1.5}`
- คะแนนต่อปุ่ม: `10 × (1 - |เวลากด - กลาง window| / ครึ่ง window)` ปัดลง — บรรทัดเดียว
